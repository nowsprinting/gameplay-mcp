...
- **Easy to extend** — The MCP server is built on [MCP C# SDK](https://github.com/modelcontextprotocol/csharp-sdk). Adding custom tools is as simple as adding `[McpServerToolType]` to a class and `[McpServerTool]` to a method.
- **Works with IL2CPP builds** — You can ship the MCP server in your player build, including IL2CPP standalone builds.
...
- [MCP C# SDK v1.0.0](https://www.nuget.org/packages/ModelContextProtocol) (NuGet)
...
`McpConfig` exposes additional settings beyond `OperatorPool`, including `GameObjectFinder`, `IsInteractable` (a `Func<Component, bool>` strategy), and `ReachableStrategy`. Refer to the [UI Test Helper](https://github.com/nowsprinting/test-helper.ui) documentation for details on these configuration options.

To stop the server from a debug menu:

```csharp
McpServer.DisposeInstance();
```

You can override the listen prefix via the `-gameplayMcpListenPrefix` command-line argument. The default is `http://+:8010/`. Note that if `ListenPrefix` is set in `McpConfig`, it takes precedence over the command-line argument.
...
- **Screen layout and navigation** — Even if the instructions describe high-level goals rather than step-by-step procedures, the AI needs to know the screen structure and which buttons navigate to which screens.
...
### Screenshot format recommendations

The default format is JPEG with quality 75, which works for most cases. Consider adjusting in these situations:
...
- **Use PNG** — PNG is preferred in the following cases:
  ...
  - When pixel-level accuracy is required for debugging purposes.
